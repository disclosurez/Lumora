...
import com.lumora.scraper.extractors.ApiVoirFilmExtractor
import com.lumora.scraper.extractors.Extractor
import com.lumora.scraper.extractors.OnRegardeOuExtractor
...
object UnJourUnFilmProvider : Provider, ProviderPortalUrl, ProviderConfigUrl {
    ...
    override suspend fun getHome(): List<Category> {
        ...
        document.select("header")
            // See AniWorldProvider - Elements.filter(NodeFilter) shadows Iterable.filter.
            .toList()
    ...
    override suspend fun getEpisodesBySeason(seasonId: String): List<Episode> {
        ...
        // The indexing has to hang off the safe call, not off its nullable result - jsoup's
        // select() returns Elements, and `x?.select(...)[i]` indexes the nullable.
        val season = document.selectFirst("div#seasons")
            ?.select("div.se-c")?.get(seasonNum.toInt())
