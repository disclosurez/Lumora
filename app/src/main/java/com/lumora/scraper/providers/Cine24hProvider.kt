...
import com.lumora.scraper.utils.WebViewResolver
...
object Cine24hProvider : Provider {
    ...
    private fun getResolver(): WebViewResolver {
        return webViewResolver ?: WebViewResolver(ScraperApp.instance).also {
            webViewResolver = it
        }
    }
    ...
    override suspend fun getHome(): List<Category> = providerMutex.withLock {
        ...
        coroutineScope {
            try {
                ...
                // Pins the try/catch to Unit. Without it the try branch ends on an else-less `if`
                // while the catch branch ends on Log.e (which returns Int), and the whole
                // try/catch is coroutineScope's return value - so the compiler tries to read the
                // `if` as an expression and demands an else. Nothing here has a useful value;
                // the result is accumulated into `categories`.
                Unit
            } catch (e: Exception) {
