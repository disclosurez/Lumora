...
object CineCalidadProvider : Provider {
    ...
    override suspend fun getEpisodesBySeason(seasonId: String): List<Episode> {
        ...
        // See AniWorldProvider - Elements.filter(NodeFilter) shadows Iterable.filter on
        // Kotlin 1.9, so go through a plain List.
        return document.select(".mark-1").toList().filter {
