...
object LaCartoonsProvider : Provider {
    ...
    private fun parseHomeShows(doc: Document): List<TvShow> {
        ...
        for (container in containers) {
            ...
            for (a in links) {
            // `x.ifBlank { continue }` is a non-local continue out of an inline lambda, which
            // Kotlin only allows from 2.1 onward; this project is on 1.9.
    ...
    override suspend fun getEpisodesBySeason(seasonId: String): List<Episode> {
        ...
        for (a in episodeLinks) {
            // See above - non-local continue out of ifBlank needs Kotlin 2.1.
            val href = a.attr("href")
            if (href.isBlank()) continue
