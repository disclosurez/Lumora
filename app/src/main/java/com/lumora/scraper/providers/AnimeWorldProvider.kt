...
object AnimeWorldProvider : Provider {
    ...
    // object is constructed by Provider.providers before that manifest has necessarily loaded.
    private val serviceHolder = HostScopedService({ baseUrl }) { AnimeWorldService.build() }

    // Set only by the SSL fallback below, and takes precedence once it is - that variant is a
    // deliberate downgrade for this site's certificate chain, so it must not be silently
    // discarded when the holder rebuilds.
    private var unsafeService: AnimeWorldService? = null

    private val service: AnimeWorldService get() = unsafeService ?: serviceHolder.get()

    private fun rebuildServiceUnsafe() {
        unsafeService = AnimeWorldService.buildUnsafe()
    }
