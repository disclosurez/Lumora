...
object FrenchMangaProvider : Provider, ProviderPortalUrl, ProviderConfigUrl {
    ...
    override val baseUrl: String
        get() {
            ...
            return cacheURL.ifEmpty { FrenchMangaProvider.defaultBaseUrl }
        }
    ...
    override suspend fun getHome(): List<Category> {
        ...
        document.select("div.sect").mapNotNull { cat_item ->
            ...
            // mapNotNull needs the lambda to produce a value, and the else-less `if` above is not
            // one. The mapped list is discarded - this loop exists for its effect on
            // `categories` - so Unit is what it produces.
            Unit
        }
