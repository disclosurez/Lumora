...
object MkissaProvider : Provider {
    ...
    override val logo: String get() = "$baseUrl/favicon-32x32.png"
    ...
    // compile-time constant. Everything the API rejects the request without still gets sent -
    // just from the client instead of the interface.
    private fun hostHeaderInterceptor() = okhttp3.Interceptor { chain ->
        val host = baseUrl
        chain.proceed(
...
//            .filterNot { it.isKnownDeadEmbedSource() }
    ...
    private fun fetchBundleCryptoConfig(html: String): BundleCryptoConfig? {
        ...
        val pageUrl = "$baseUrl/anime".toHttpUrlOrNull() ?: return null
