...
import com.lumora.scraper.database.AniWorldDatabase
...
import com.lumora.scraper.providers.SerienStreamProvider.SerienStreamService
...
object AniWorldProvider : Provider {
    ...
    override val logo: String get() = "$URL/public/img/facebook.jpg"
    override val language = "de"

    // Rebuilt when the manifest repoints this site - see HostScopedService.
    private val serviceHolder = HostScopedService({ baseUrl }) { Service.build() }
    ...
    /**
     * Upstream scheduled a one-shot WorkManager job here that back-filled posters and overviews
     * for the cached show list from an `aniworld_tvshows.json` asset bundled in that app. Lumora
     * does not ship that asset - it was a several-megabyte snapshot of the site's catalogue, and
     * a snapshot goes stale - so there is nothing for the worker to read.
     *
     * Titles without a poster in the live catalogue simply come back without one, the same as
     * they would upstream on a build where the asset was absent. Left as a no-op rather than
     * deleted so the call site keeps documenting where the seeding used to happen.
     */
    ...
    override suspend fun getTvShow(id: String): TvShow {
        ...
        val tvShow = TvShow(
            ...
            seasons = document.select("#stream > ul:nth-child(1) > li")
                // .toList() first throughout this file and its siblings: jsoup's Elements
                // declares its own filter(NodeFilter) member, and a member beats an extension,
                // so `elements.filter { }` binds to that instead of Iterable.filter and demands
                // a two-argument NodeFilter lambda. Kotlin 2.x resolves this the intuitive way;
                // this project is on 1.9, where the member still wins.
                .toList()
