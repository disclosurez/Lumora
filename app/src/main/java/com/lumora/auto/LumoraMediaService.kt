 ...
 *
 * This exists alongside [com.lumora.auto.legacy.LegacyCarService] rather than instead of it.
 * The two are listed by different halves of the car host and do different jobs - this is the
 * fcaronte/AABrowser pairing, which is what makes the host show the app twice:
 *
 ...
 *  - The legacy projection service draws the real player UI, but whether the host lists a
 *    sideloaded projection app is up to the host, and a version that declines to leaves the
 *    user with audio only.
 *
 * So: media guarantees the app is usable in the car, and the projection service is the
 * upgrade when the host allows it.
 */
