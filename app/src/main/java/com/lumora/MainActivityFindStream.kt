...
private fun FoundSource.key(): String = when (this) {
...
internal fun MainActivity.showFindStreamDialog(
    ...
    val siteProviders = ScraperSiteStore.activeProviders(this)
    ...
    val prefs = getSharedPreferences(FIND_STREAM_PREFS, Context.MODE_PRIVATE)
    val lastGood = prefs.getString(KEY_LAST_GOOD, null)
    val preferDub = this.prefs.getBoolean(PREF_PREFER_DUB_AUDIO, false)
    ...
    val status = TextView(this).apply {
        ...
        setTextColor(ContextCompat.getColor(this@showFindStreamDialog, R.color.text_primary))
    }
    val detail = TextView(this).apply {
        setTextColor(ContextCompat.getColor(this@showFindStreamDialog, R.color.text_secondary))
        textSize = 12f
    ...
    fun clearWatch() {
        ...
        watchdog = null
        watching?.let { playerManager.getExoPlayer().removeListener(it) }
        watching = null
    }
    ...
    fun startPlayback(source: FoundSource, url: String, headers: Map<String, String>,
        ...
        showPlayerFor(
            ...
            externalSubtitles = subtitles.map(::externalSubtitleFor),
        ...
        watching = listener
        playerManager.getExoPlayer().addListener(listener)
        val timeout = Runnable {
            // Left the player, or it recovered on its own - either way this is not ours to act on.
            if (!isPlayerVisible || playerManager.playbackState == androidx.media3.common.Player.STATE_READY) {
                confirmPlayback()
                return@Runnable
            }
            clearWatch()
        ...
        watchdog = timeout
        mainHandler.postDelayed(timeout, STALL_TIMEOUT_MS)
    }
    ...
    fun tryNext() {
        ...
        if (next == null) {
            if (searchesDone >= expectedSearches) status.text = getString(R.string.scraper_no_sources)
            return
        }
        trying = true
        tried += next.key()
        attempts++
        status.text = getString(R.string.scraper_opening_source, next.title)
        detail.visibility = View.VISIBLE
        detail.text = getString(R.string.scraper_attempt_detail, next.detail, attempts)

        scope.launch {
            when (next) {
                is FoundSource.FromPlugin -> {
                    val resolved = if (next.plugin.resolvesNatively) {
                        resolveTorrentStream(next.result.token, season, episode) { line ->
                            runOnUiThread { if (!playbackStarted) detail.text = line }
                        }
                    } else {
                        jsPluginEngine.resolve(
                            pluginScriptManager.readSource(next.plugin),
            ...
            launch {
                runCatching {
                    jsPluginEngine.runSearch(
                        source = pluginScriptManager.readSource(plugin),
                        query = item.name,
                        year = item.year?.toIntOrNull(),
                        season = season,
                        episode = episode,
                        onProgress = { line -> runOnUiThread { if (found.isEmpty()) status.text = line } },
                        onResult = { result -> onFound(FoundSource.FromPlugin(plugin, result)) },
                    )
                }
                runOnUiThread { searchesDone++; tryNext() }
            }
        }
                    ...
                    ScraperCatalog.findSources(
                        ...
                        onProgress = { searched, total ->
                            runOnUiThread {
                                if (found.isEmpty()) {
                                    status.text =
                                        getString(R.string.scraper_searching_sites, searched, total)
                                }
                            }
                        },
 ...
 */
private fun ranking(lastGood: String?, preferDub: Boolean): Comparator<FoundSource> =
